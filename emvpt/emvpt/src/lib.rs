...
impl EmvConnection<'_> {
    ...
    pub fn handle_verify_enciphered_pin(&mut self, ascii_pin: &[u8]) -> Result<(), ()> {
        ...
        debug!("icc_pin_pk: {:?}", self.icc.icc_pin_pk);
        debug!("plaintext_data: {:?}", plaintext_data);
        debug!("ciphered_pin_data: {:?}", ciphered_pin_data);
    ...
    fn validate_ac(&self, requested_cryptogram_type: CryptogramType) -> Result<CryptogramType, ()> {
        ...
        debug!("_tag_9f36_application_transaction_counter: {:?}", _tag_9f36_application_transaction_counter);
        debug!("_tag_9f26_application_cryptogram: {:?}", _tag_9f26_application_cryptogram);
        debug!("_tag_9f10_issuer_application_data: {:?}", _tag_9f10_issuer_application_data);
        debug!("icc_cryptogram_type: {:?}", icc_cryptogram_type);
    ...
    fn send_generate_ac(
        ...
        if self.icc.capabilities.cda {
            ...
            if let None = self.get_tag_value("9F4C") {
                ...
                debug!("tag_9f4c_icc_dynamic_number: {:?}", tag_9f4c_icc_dynamic_number);
        ...
        debug!("calling validate ac with: {:?}", requested_cryptogram_type);
    ...
    pub fn handle_1st_generate_ac(&mut self) -> Result<CryptogramType, ()> {
        ...
        if self.contactless && self.get_tag_value("9F26").is_some() {
            ...
            debug!("icc_cryptogram_type: {:?}", icc_cryptogram_type);
            debug!("settings.terminal.cryptogram_type: {:?}", self.settings.terminal.cryptogram_type);
    ...
    pub fn handle_public_keys(&mut self, application: &EmvApplication) -> Result<(), ()> {
        ...
        debug!("------ISSUER PUBLIC KEY------");
        debug!("Issuer PK Modulus: {:?}", issuer_pk_modulus);
        debug!("Issuer PK Exponent: {:?}", issuer_pk_exponent);
        debug!("Censor Sensitive Fields: {:?}", self.settings.censor_sensitive_fields);
        ...
        debug!("Data Authentication: {:?}", data_authentication);

        let tag_9f46_icc_pk_certificate = self.get_tag_value("9F46");
        let tag_9f47_icc_pk_exponent = self.get_tag_value("9F47");

        debug!("ICC PK Certificate: {:?}", tag_9f46_icc_pk_certificate);
        debug!("ICC PK Exponent: {:?}", tag_9f47_icc_pk_exponent);
    ...
    pub fn get_issuer_public_key(
        ...
        log::info!("get_issuer_public_key");
        ...
        log::info!("CA PK Modulus: {:?}", ca_pk.modulus);
        log::info!("CA PK Exponent: {:?}", ca_pk.exponent);
        log::info!("Tag 90 Issuer Public Key Certificate: {:?}", tag_90_issuer_public_key_certificate);

        let issuer_certificate = ca_pk
            .public_decrypt(&tag_90_issuer_public_key_certificate[..])
            .unwrap();

        log::info!("Issuer Certificate: {:02X?}", issuer_certificate);
    ...
    pub fn validate_signed_dynamic_application_data(
        ...
        debug!("VALIDATE SIGNED DYNAMIC APPLICATION DATA (SDAD):");
        ...
        debug!(
            "tag_9f4b_signed_data_decrypted: {:?}",
            tag_9f4b_signed_data_decrypted
        );
        ...
        debug!(
            "tag_9f4b_signed_data_decrypted_dynamic_data: {:?}",
            tag_9f4b_signed_data_decrypted_dynamic_data
        );
    ...
    pub fn handle_signed_static_application_data(
        ...
        debug!("tag_93_ssad_decrypted: {:?}", tag_93_ssad_decrypted);
        debug!("tag_93_ssad: {:?}", tag_93_ssad);
        debug!("icc.issuer_pk: {:?}", self.icc.issuer_pk);
    ...
    pub fn handle_card_verification_methods(&mut self) -> Result<(), ()> {
        ...
        debug!("------CARD VERIFICATION METHOD------");
        debug!("Purchase amount: {}", purchase_amount);

        let cvm_rules = self.icc.cvm_rules.clone();

        debug!("CVM Rules: {:?}", cvm_rules);
    ...
    pub fn handle_offline_data_authentication(&mut self) -> Result<(), ()> {
        ...
        debug!("------OFFLINE DATA AUTHENTICATION------");
    ...
    pub fn handle_issuer_authentication_data(&mut self) -> Result<(), ()> {
        ...
        debug!("tag_91_issuer_authentication_data: {:?}", tag_91_issuer_authentication_data);
...
impl RsaPublicKey {
    ...
    pub fn public_encrypt(&self, plaintext_data: &[u8]) -> Result<Vec<u8>, ()> {
        ...
        if let Some(true) = self.sensitive {
            debug!("Encrypt result ({} bytes)", data.len());
        } else {
            debug!(
    ...
    pub fn public_decrypt(&self, cipher_data: &[u8]) -> Result<Vec<u8>, ()> {
        ...
        debug!("PUBLIC_DECRYPT");

        let pk_modulus_raw = hex::decode(&self.modulus).unwrap();
        let pk_modulus = BigNum::from_slice(&pk_modulus_raw[..]).unwrap();
        let pk_exponent = BigNum::from_slice(&(hex::decode(&self.exponent).unwrap())[..]).unwrap();

        debug!("PK Modulus: {:?}", pk_modulus);
        debug!("PK Exponent: {:?}", pk_exponent);
        debug!("Cipher data: {:?}", cipher_data);
        ...
        // debug!(
        //     "Decrypt output ({} bytes):\n{}",
        //     decrypt_output.len(),
        //     HexViewBuilder::new(&decrypt_output[..]).finish()
        // );
        debug!("Decrypt output ({} bytes): {:?}", decrypt_output.len(), decrypt_output);
        ...
        //     HexViewBuilder::new(&data[..]).finish()
        // );
        debug!("Data ({} bytes): {:?}", data.len(), data);


        if let Some(true) = self.sensitive {
            debug!("Decrypt result ({} bytes)", data.len());
        } else {
            debug!(
