...
const pemFormat = `-----BEGIN PUBLIC KEY-----\n${Buffer.from(`3056301006072a8648ce3d020106052b8104000a03420004${Buffer.from(modulus, "hex").toString("base64")}${Buffer.from(exponent, "hex").toString("base64")}`, "base64").toString("base64").match(/.{1,64}/g).join("\n")}\n-----END PUBLIC KEY-----`;

console.log(pemFormat);

// Convert the public key components into a PEM format key
const publicKeyPem = `-----BEGIN PUBLIC KEY-----\n${Buffer.from(
    `MIIBIjANBgkqhkiG9w0BAQEFAAOCAQ8AMIIBCgKCAQE=${Buffer.from(modulus).toString('base64')}`,
    'base64'
).toString('base64')}\n-----END PUBLIC KEY-----`;

console.log('publicKeyPem', publicKeyPem)
// Convert cipher array to a buffer
// const cipherBuffer = Buffer.from(cipher);

// // Use the last 20 bytes of the message (already a hash)
// const hashOfMessage = Buffer.from(message.slice(-20));

// // Verify the signature
// const verifier = crypto.createVerify('RSA-SHA256');
// verifier.update(hashOfMessage);

// const isValid = verifier.verify(publicKeyPem, cipherBuffer);
